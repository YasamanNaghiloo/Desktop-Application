...
/**
 * Controls the add to shopping list scene.
 */
public class AddToShoppingListController extends RouteController {
  ...
  public void initialize() {
    ...
    SpinnerValueFactory<Double> valueFactory = new SpinnerValueFactory.DoubleSpinnerValueFactory(0.001, 1000000, 0.5,
        0.5);
    ...
    if (units.containsKey(unitComboBox.getValue())) {
      if (ingredients.containsKey(ingredientComboBox.getValue())) {
        try {
          ...
          System.err.println("hahahahahha");
          shoppingListDao.inputListIngredient(t, false);
          getNavigator().showDialog("Success", "Ingredient successfully added");
          ShoppingListViewController shoppingListViewController = (ShoppingListViewController) getNavigator()
              .getHistoryItem(Route.SHOPPING_LIST)
              .getController();
          shoppingListViewController.initialize();
        ...
        if (ingredientComboBox.getValue() != null && ingredientComboBox.getValue() != "") {
          try {
            TripleTuple<Ingredient, Double, Unit> t = new TripleTuple<Ingredient, Double, Unit>(
                ...
                quantitySpinner.getValue(),
                new Unit(units.get(unitComboBox.getValue()), unitComboBox.getValue()));
            shoppingListDao.inputListIngredient(t, true);
